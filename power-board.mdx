...
The monitor task provides intelligent overcurrent detection and automatic clearing through a **dual-expander two-stage architecture**.

### Hardware Architecture

**Two-Stage Detection Chain (Active-Low Logic):**
...
The firmware uses a two-stage I2C read approach for efficiency:

**Stage 1 - Fast Pre-Filter (1 I2C transaction):**
```cpp
...
if (!any_overcurrent) return;  // No faults detected, done
```

**Stage 2 - Channel Identification (Only if Stage 1 triggered):**
```cpp
...
- **99% of polls use only 1 I2C transaction** (Stage 1 returns no fault)
- **Stage 2 only runs when overcurrent detected** (rare event)
- **Software latching** ensures faults aren't missed between polls

### Automatic Clearing Sequence

When an overcurrent is detected, the system automatically attempts to clear it:

**High-Level Flow:**
1. **Detection** - Monitor task detects fault via two-stage polling (above)
2. **Latching** - Fault bit set in `s_snapshot.oc_mask` (bit 0-3 for channels 0-3)
3. **Delay** - Waits 5 seconds (configurable) before first clear attempt
4. **Clearing** - 3-step hardware clear sequence (below)
5. **Retry** - If clear fails, retries every 60 seconds up to 10 attempts
6. **Logging** - All events logged with ESP_LOG for debugging

**Detailed 3-Step Clear Sequence:**

```cpp
// Step 1: Read INA237 DIAG_ALERT register (clears INA237 internal latch)
uint16_t diag_alert;
read_ina237_reg(ina_addr[ch], INA237_REG_DIAG_ALERT, &diag_alert);
// Bit 6 = overcurrent flag (reading register clears it)

// Step 2: Send STM32C0 clear command (clears A3942 motor driver fault)
watts_stm_send_command(0x11 + ch);  // Commands: 0x11, 0x12, 0x13, 0x14
// Clears hardware faults: short circuit, open load, thermal shutdown

// Step 3: Poll hardware ALERT pin for 500ms to verify stable clear
int polls_ok = 0;
int polls_total = 0;
for (500ms at 10ms interval) {  // 50 polls total
    uint8_t p1_power = read_i2c(0x21, TCA_INPUT_P1);
    bool alert_clear = ((p1_power & (1 << ch)) != 0);  // HIGH = cleared
...
// Require 80% of polls to be HIGH to confirm stable clear
int required = (polls_total * 4) / 5;
if (polls_ok >= required && polls_total > 0) {
    s_snapshot.oc_mask &= ~(1 << ch);  // Clear latched bit
    ESP_LOGI("POWER_MON", "Channel %d overcurrent CLEARED", ch);
} else {
    ESP_LOGW("POWER_MON", "CH%d: Clear FAILED (will retry)", ch);
}
```
...
if (watts_stm_read_status(&status)) {
    ...
    if (status & 0x02) Serial.println("A3942 fault detected");
    if (status & 0x10) Serial.println("Overcurrent detected");
}

// Read per-channel fault codes
uint8_t fault;
if (watts_stm_read_fault(2, &fault)) {
    if (fault & 0x01) Serial.println("CH2: Short to Ground");
    if (fault & 0x02) Serial.println("CH2: Short to Battery");
    if (fault & 0x04) Serial.println("CH2: Open Load");
    if (fault & 0x08) Serial.println("CH2: Overtemperature Warning");
    if (fault & 0x10) Serial.println("CH2: CP Undervoltage");
}
```

### A3942 Fault Decoding

The A3942 fault register uses bits D4:D0 (lower 5 bits). Bits D7:D6 contain the channel address.

| Bit | Mask | Fault Code | Description |
|-----|------|------------|-------------|
| D0 | 0x01 | [STG] | Short to Ground - Output shorted to GND |
| D1 | 0x02 | [STB] | Short to Battery - Output shorted to V+ |
| D2 | 0x04 | [OL] | Open Load - No LED strip connected |
| D3 | 0x08 | [TWARN] | Overtemperature Warning |
| D4 | 0x10 | [CP_UVLO] | Charge Pump Undervoltage Lockout |
| D7:D6 | 0xC0 | - | Channel address (0-3) |
...
**Source Code:** [src/power super/Core/Src/main.c](src/power%20super/Core/Src/main.c)

### System Architecture

The STM32 and ESP32-C5 have distinct responsibilities regardless of operating mode:

```
┌─────────────────────────────────────────────────────────────────┐
│ STM32C0 (Always at Boot)                                        │
│ Source: src/power super/Core/Src/main.c                         │
├─────────────────────────────────────────────────────────────────┤
│ • Configures INA237 sensors (ADC_CONFIG, SHUNT_CAL, SOVL)       │
│ • Configures A3942 motor driver via SPI                         │
│ • Reads hardware switches (P0.0-P0.2) for mode selection        │
│ • Sets overcurrent thresholds based on P0.0 (5A or 8A mode)     │
│ • Performs preflight load detection                             │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│ ESP32-C5 (Always)                                               │
│ Source: src/Watts-ESP-IDF-C5/components/watts_power_board/      │
├─────────────────────────────────────────────────────────────────┤
│ • Configures TCA9555 GPIO expander (NOT INA237)                 │
│ • READS INA237 measurements (current, voltage, power, temp)     │
│ • Controls W5500 Ethernet reset via expander                    │
│ • Provides API for WLED integration                             │
└─────────────────────────────────────────────────────────────────┘
```

**Mode-Specific Behavior:**

| Responsibility | STANDALONE (P0.1=1) | PERIPHERAL (P0.1=0) |
|---------------|---------------------|---------------------|
| **INA237 Configuration** | STM32 | STM32 |
| **A3942 Control** | STM32 (autonomous) | STM32 (via I2C commands from ESP32) |
| **OC Detection** | STM32 polls ALERT pins | ESP32 polls ALERT pins via expander |
| **OC Clear Sequence** | STM32 handles | ESP32 handles |
| **OC Poll Duration** | 500ms | 1000ms |
| **First Clear Delay** | Immediate | 5000ms (configurable) |
| **Keep-Alive Check** | STM32 (P0.2 can bypass) | STM32 (always enforced) |

### Operating Modes

The STM32 reads configuration pins from the TCA9555 GPIO expander at boot to determine its operating mode.

**Source:** [main.c:313-337](src/power%20super/Core/Src/main.c#L313-L337) - TCA9555_Init()

#### Configuration Switches

Three hardware configuration switches on the power board control the STM32 firmware behavior. These switches connect to TCA9555 Port 0 inputs (P0.0, P0.1, P0.2) and are read at startup:
...
**Source Code:**
- STM32 I2C slave: [main.c:759-822](src/power%20super/Core/Src/main.c#L759-L822) - I2C callbacks
- STM32 command processing: [main.c:861-934](src/power%20super/Core/Src/main.c#L861-L934) - Process_Peripheral_Command()
- ESP32 command sender: [watts_power_board.cpp:311-338](src/Watts-ESP-IDF-C5/components/watts_power_board/watts_power_board.cpp#L311-L338)
...
**I2C Command Set (defined in [main.c:57-77](src/power%20super/Core/Src/main.c#L57-L77)):**
...
**Source Code:**
- Main loop: [main.c:1023-1070](src/power%20super/Core/Src/main.c#L1023-L1070) - STANDALONE mode loop
- OC polling: [main.c:551-617](src/power%20super/Core/Src/main.c#L551-L617) - Poll_Overcurrent()
- OC handling: [main.c:619-701](src/power%20super/Core/Src/main.c#L619-L701) - Handle_Overcurrent()
...
Both STANDALONE and PERIPHERAL modes implement automatic overcurrent recovery, but with different timing and handlers.

**Recovery Parameters by Mode:**

| Parameter | STANDALONE (STM32) | PERIPHERAL (ESP32) |
|-----------|-------------------|-------------------|
| First clear delay | Immediate | 5000ms |
| Retry interval | 60s | 60s |
| Max attempts | 10 | 10 |
| Poll duration | 500ms | 1000ms |
| Poll interval | 50ms | 50ms |
| Success threshold | 80% | 80% |

**Source Code:**
- STANDALONE: [main.c:141-145](src/power%20super/Core/Src/main.c#L141-L145) - STM32 timing defines
- PERIPHERAL: [watts_power_board.h:24-28](src/Watts-ESP-IDF-C5/components/watts_power_board/include/watts_power_board.h#L24-L28) - ESP32 timing defines

**STM32 STANDALONE Recovery Parameters:**
```c
// Source: main.c:141-145
...
**ESP32 PERIPHERAL Recovery Parameters:**
```c
// Source: watts_power_board.h:24-28
#define WATTS_OC_FIRST_CLEAR_DELAY_MS   5000   // Wait 5s before first clear
#define WATTS_OC_RETRY_INTERVAL_MS      60000  // Retry every 60s
#define WATTS_OC_MAX_ATTEMPTS           10     // Give up after 10 attempts
#define WATTS_OC_POLL_DURATION_MS       1000   // Poll ALERT for 1s after clear
#define WATTS_OC_POLL_INTERVAL_MS       50     // Poll every 50ms during verify
```
...
The STM32 configures all 4 INA237 power monitors with overcurrent thresholds based on the current mode setting (P0.0 hardware switch).

**Source Code:** [main.c:343-383](src/power%20super/Core/Src/main.c#L343-L383) - INA237_Init_All()

**Configuration:**
```c
// Source: main.c:347-350
...
The SOVL register uses **shunt voltage threshold** (not current directly). With ADCRANGE=0, the LSB is 5 µV:

```
SOVL Formula: Threshold Current = (raw × 5µV) / R_shunt

5A Mode: oc_threshold = 10000
  → Shunt voltage: 10000 × 5µV = 50mV
  → Current threshold: 50mV / 10mΩ = 5.0A

8A Mode: oc_threshold = 16000
  → Shunt voltage: 16000 × 5µV = 80mV
  → Current threshold: 80mV / 10mΩ = 8.0A
```

Note: The Current LSB (305µA) is used for current register readings, not for SOVL threshold configuration.
...
- [W5500 Ethernet Controller](w5500-ethernet.mdx) - Full W5500 driver documentation with recovery system
