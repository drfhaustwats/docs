...
if (forceResetCount >= 3) {
    gpio_set_direction(WROOM_RST_PIN, GPIO_MODE_OUTPUT);  // GPIO26
    gpio_set_level(WROOM_RST_PIN, 0);   // Assert reset (active LOW)
    vTaskDelay(pdMS_TO_TICKS(200));     // Hold 200ms
    gpio_set_level(WROOM_RST_PIN, 1);   // Release reset
    gpio_set_direction(WROOM_RST_PIN, GPIO_MODE_INPUT);
}
```
...
parlio_tx_gpio_config_t gpio_config = {
    .clk_out_pin = PINS.spi.sclk,      // Clock to WROOM
    .valid_pin = GPIO_NUM_25,           // Frame valid signal
    .data_pins = {PINS.hardware.led.pin[2]},  // Data pin (WLED output 3)
};
```
...
|-----|----------|-----------|-------------|
| `PINS.spi.sclk` | ParlIO Clock | Output | 30MHz clock to ESP32 WROOM |
| `GPIO_NUM_25` | ParlIO Valid | Output | Frame valid signal |
| `PINS.hardware.led.pin[2]` | ParlIO Data | Output | Serial data (WLED output 3) |
| `PINS.spi.miso` | Handshake | Input | WROOM READY/DONE signal |
| `GPIO_NUM_26` | WROOM Reset | Bidir | Hardware reset control |
...
1. **Hardware Configuration**:
   ...
   - Ensure output 3 pin matches `PINS.hardware.led.pin[2]` (used for ParlIO data)
...
- Check physical connections: ParlIO clock, data, handshake GPIO
- Verify WROOM_RST_PIN (GPIO26) connection
