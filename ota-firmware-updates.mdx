...
The WROOM update process follows a 6-state machine defined in `h2_ota.h`:
...
- WROOM validates firmware header (version check, SHA-256)
- After validation succeeds and `esp_ota_begin()` completes, WROOM sends ACK: `+OTAACK\r\n`
- C5 waits for this ACK with 5-second timeout before continuing
- Subsequent chunks are streamed without waiting for ACK (fire-and-forget mode)
...
### WROOM-Side OTA Process

The WROOM firmware ([ota.c](src/ESP32 Wroom LED Driver firmware/main/ota.c)) implements the receive side:

1. **AT Command Handler**: Receives `AT+OTAUPDATE=<chunk_size>` and spawns OTA task
2. **Sync Header Wait**: Expects 6-byte sync header `{0xC0, 0xFF, 0xFE, 0xAA, 0x55, 0x90}` within 5 seconds
3. **Header Validation**: First chunk validates:
   - Firmware version (must differ from running version)
   - SHA-256 hash of incoming image
   - Ensures version doesn't match last-invalid partition
4. **ACK Transmission**: Sends `+OTAACK\r\n` after successful `esp_ota_begin()`
5. **Streaming Reception**: Receives remaining chunks with 200ms read timeout per chunk
6. **Completion**: After 3-second idle timeout, calls `esp_ota_end()` and reboots

**WROOM Error Codes** (sent as `+OTAERR:<code>`, defined in `ota.h`):
- `1` - SYNC_TIMEOUT: Sync header not received or mismatch
- `2` - VERSION_ABORT: Version identical to running or last-invalid
- `3` - UART: UART read/write error
- `4` - FLASH_WRITE: `esp_ota_write()` failed
- `5` - VALIDATE_FAILED: `esp_ota_end()` validation failed
- `6` - STREAM_TIMEOUT: No new bytes received during streaming
- `7` - UNEXPECTED_PACKET: Reserved (not used currently)
- `8` - UNKNOWN: Other errors (e.g., buffer allocation failure)
...
|-----------|------|-------------|
| **C5 OTA State Machine** | `h2_ota.cpp` | `processH2OTAStateMachine()`, chunk download/transmit |
| **C5 OTA Header** | `h2_ota.h` | `h2_ota_state_t` enum, API declarations |
| **WROOM OTA Receiver** | `ota.c` | Binary stream reception, flash writing |
| **WROOM Error Codes** | `ota.h` | `OTA_ERR_*` error code definitions |
| **MQTT Commands** | `mqtt.cpp` | `updateFirmware`, `updateH2Firmware` handlers |
