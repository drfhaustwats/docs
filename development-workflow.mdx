...
- ESP32 WROOM development board with USB-C connector
...
### ESP32 WROOM ESP-IDF Environment
...
1. **UART TX (C5→WROOM)** - AT commands
2. **UART RX (WROOM→C5)** - AT responses
...
class ATCommandTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.serial = serial.Serial('/dev/ttyUSB1', 921600, timeout=2)
        time.sleep(1)  # Allow WROOM to initialize
    ...
    def test_led_commands(self):
        ...
        # Prepare LED frame (8 params: 4 pixel counts + 4 formats)
        # Format: AT+LED_PREP=px0,px1,px2,px3,fmt0,fmt1,fmt2,fmt3
        # Formats: 3=RGB, 4=RGBW
        self._send_command("AT+LED_PREP=100,200,150,250,3,3,3,3")
        response = self._read_response()
        self.assertIn("OK", response)
...
class PerformanceTests:
    def __init__(self, h2_port):
        self.h2_serial = serial.Serial(h2_port, 921600, timeout=1)
    ...
    def test_led_frame_throughput(self, frame_count=50):
        ...
        # Test with different pixel configurations (ch0, ch1, ch2, ch3)
        pixel_configs = [
            (100, 100, 0, 0),      # Channels 0-1 only
            (500, 500, 0, 0),      # Large channels 0-1
            (100, 100, 100, 100)   # All 4 channels
        ]

        for px0, px1, px2, px3 in pixel_configs:
            total_pixels = px0 + px1 + px2 + px3
            print(f"Testing {total_pixels} pixels ({px0},{px1},{px2},{px3})...")

            # Prepare LED controller (8 params: pixel counts + formats)
            # Using RGB (format=3) for all channels
            cmd = f"AT+LED_PREP={px0},{px1},{px2},{px3},3,3,3,3\r\n"
            ...
            pixels_per_second = total_pixels * frames_per_second
