...
These constants are defined in `led_detect.cpp`:
...
Detection can be triggered via MQTT command:
...
The WLED web server provides a `/detectLeds` endpoint:
...
```json
{
  "tested": 4,
  "results": [
    {"ch": 0, "count": 150, "type": 30, "baseline": 180, "delta": 8},
    {"ch": 1, "count": 200, "type": 22, "baseline": 150, "delta": 6},
    {"ch": 2, "count": 100, "type": 30, "baseline": 120, "delta": 9},
    {"ch": 3, "count": 0, "type": 30, "baseline": 50, "delta": 0}
  ]
}
```
...
{
  "ch": 0,
  "error": "No measurable LED[0] delta (3 mA)"
}
```
...
|-----------|------|-------------|
| **Detection Core** | `led_detect.cpp` | Binary search algorithm, current measurement |
| **Detection API** | `led_detect.h` | `LedDetectResult`, `led_detect_run()` interfaces |
| **HTTP Endpoint** | `wled_server.cpp` | `/detectLeds` REST API handler |
| **MQTT Handler** | `mqtt.cpp` | `detectLeds` command processing |
| **Power Board** | `watts_power_board.cpp` | Current measurement via INA237 |
