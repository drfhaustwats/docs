...
parlio_tx_gpio_config_t gpio_config = {
    .clk_out_pin = PINS.spi.sclk,  // GPIO 23: 30MHz clock to ESP32 WROOM
    .valid_pin = PINS.spi.cs,      // GPIO 28: Frame valid signal (active high)
    .data_pins = {PINS.spi.mosi}   // GPIO 8: Serial data
};
```
...
spi_bus_config_t buscfg = {
    .mosi_io_num = PINS.spis.mosi,       // GPIO 23: Data from C5 GPIO 8
    .sclk_io_num = PINS.spis.sck,        // GPIO 18: Clock from C5 GPIO 23
    .max_transfer_sz = 16012,            // Maximum frame size (4 channels)
...
spi_slave_interface_config_t slvcfg = {
    ...
    .spics_io_num = PINS.spis.cs,        // GPIO 5: Valid/CS from C5 GPIO 28
...
- **Pre-allocated Buffers**: 8 × 16KB DMA-capable buffers
...
DATA (Serial)     →   MOSI (Serial)
VALID (Frame)     →   CS (Active High)
```
