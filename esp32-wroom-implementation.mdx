...
**Target:** ESP32 (classic WROOM module) - `CONFIG_IDF_TARGET="esp32"` (see `sdkconfig.defaults`)
...
2. **spi_rx_task** (Priority 12, 16KB stack):
   ```cpp
   static void spi_rx_task(void *param) {
       led_controller_t *ctrl = (led_controller_t*)param;
       ...
       while (1) {
           ...
           if (ret == ESP_OK) {
               ...
               process_led_frame(ctrl, t.rx_buffer, t.trans_len);
           }
       }
   }
   ```

3. **ble_host_task** (NimBLE managed):
   - Runs `nimble_port_run()`
   - Handles BLE stack operations
   - Managed by NimBLE FreeRTOS integration

4. **heartbeat_timer** (ESP Timer - 10 second interval):
   ```cpp
...
The controller uses **NeoPixelBus** library with RMT backend for LED output. NeoPixelBus handles all RMT timing automatically for WS2812/SK6812 protocols.

**Strip Factory (from spi_led_controller.cpp):**
```cpp
// RGB strips (WS2812 protocol via RMT)
static IStrip* make_rgb_strip(int ch, uint16_t pixels, gpio_num_t pin) {
    switch (ch) {
        case 0: return new StripWrap<NeoPixelBus<NeoGrbFeature, NeoEsp32Rmt0Ws2812xMethod>>(
                    new NeoPixelBus<NeoGrbFeature, NeoEsp32Rmt0Ws2812xMethod>(pixels, pin));
        // ... channels 1-3 use Rmt1/Rmt2/Rmt3 methods
    }
}
...
**Note:** For direct RMT usage (parlio_led_controller.c), the resolution is 40MHz for 25ns timing resolution.
...
**Source:** [at_led_commands.c](src/ESP32%20Wroom%20LED%20Driver%20firmware/main/at_led_commands.c) - `at_cmd_led_prep_spi()` function
...
- **Stack**: 4KB (cat_task) + 16KB (spi_rx_task) + BLE stack = ~24KB total
