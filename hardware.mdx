...
These mappings are defined in [NeoEsp32ParlC5SharedManager.h:36-42](src/Watts-ESP-IDF-C5/components/NeoPixelBus/src/internal/methods/NeoEsp32ParlC5SharedManager.h#L36-L42)
...
The power board implements intelligent overcurrent detection and automatic clearing through a dual-expander architecture:

**Hardware Detection Path:**
```
INA237 Sensors (4×)
  ↓ ALERT pins (active low)
Power Board Expander (0x21)
  - Port 1, bits 0-3 read ALERT status
  ↓ POWER_INT output (active low)
Main Board Expander (0x20)
  - Port 1, bit 7 monitors POWER_INT
  ↓ I2C polling (50ms)
ESP32-C5 Firmware
```

**Software Detection Flow:**
1. **Poll Main Expander**: Read P1.7 to check POWER_INT status
2. **If POWER_INT Active**: Read Power Board Expander P1.0-P1.3
3. **Update OC Mask**: Set bits for any channels with active ALERT
4. **Snapshot Update**: Thread-safe update of overcurrent mask
