...
1. **Tiny-CAT State Machine** (`main/cat.c`):
   ```cpp
   // cat_handle_char() - State machine processing
...
2. **Command Registration** (`main/EspAt_firmware.c`):
   ```cpp
   // register_all_commands() - Verify command table
...
1. **Response Parser** (`components/athost/src/ATHost.cpp`):
   ```cpp
   // ATHost_sendATWaitOK() - Response reading logic
...
2. **URC Processing** (`components/athost/src/ATHost.cpp`):
   ```cpp
   // processURC() - Unsolicited Result Code handling
...
1. **Shared Manager Bus Wrapper** (`components/NeoPixelBus/src/internal/methods/NeoEsp32ParlC5SharedManager.h`):
   ```cpp
   // NeoEsp32ParlC5SharedManager - ParlIO configuration
   class NeoPixelBusSharedManager {
       rmt_channel_handle_t _rmt_channels[2];     // RMT channels 0,1
       parlio_tx_unit_handle_t _parlio_unit;      // ParlIO unit for WROOM

       // initializeParlIO() - Check ParlIO initialization
...
2. **Frame Data Assembly** (`components/NeoPixelBus/src/internal/methods/NeoEsp32ParlC5SharedManager.h`):
   ```cpp
   // Frame assembly for WROOM channels
...
1. **SPI Slave Configuration** (`main/spi_led_controller.cpp`):
   ```cpp
   // led_controller_init() - SPI slave setup
...
2. **Frame Reception Task** (`main/spi_led_controller.cpp`):
   ```cpp
   // spi_rx_task() - SPI reception task
...
3. **Frame Validation** (`main/spi_led_controller.cpp`):
   ```cpp
   // validate_frame() - CRC and header validation
...
1. **AT LED_PREP Command** (`main/at_led_commands.c`):
   ```cpp
   // at_cmd_led_prep_spi() - Frame preparation timing
...
2. **NeoPixelBus Integration** (`components/NeoPixelBus/src/internal/methods/NeoEsp32ParlC5SharedManager.h`):
   ```cpp
   // show() - Synchronization with ATHost
   void show() {
...
1. **BLE Initialization** (`main/ble_at_handler.c`):
   ```cpp
   // at_cmd_bleinit() - BLE stack initialization
...
2. **Service Management** (`main/gatt_svc.c`):
   ```cpp
   // gatt_svc_dynamic_add_service() - Dynamic service registration
...
1. **Dynamic Service Creation** (`main/ble_at_handler.c`):
   ```cpp
   // create_lighting_service() - Lighting service example
...
2. **GATT Access Handler** (`main/gatt_svc.c`):
   ```cpp
   // dynamic_gatt_access_cb() - Characteristic access handling
...
1. **Watchdog Configuration** (`main/EspAt_firmware.c`):
   ```cpp
   // Watchdog setup in app_main()
   esp_task_wdt_init(30, true);              // 30 second timeout
   esp_task_wdt_add(xTaskGetCurrentTaskHandle());
   ```

2. **Error State Detection** (`components/athost/src/ATHost.cpp`):
   ```cpp
